        ...
        {leaderboard.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {leaderboard.map((entry: any, i: number) => {
              ...
              return (
                <div key={entry.id} className="flex items-center gap-3 rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0" style={{ border: "2px solid " + (entry.level_color ?? "#2A9D8F") }}>
                    {entry.photo_url
                      ...
                      : <div className="w-full h-full flex items-center justify-center text-white font-black text-sm" style={{ background: entry.level_color ?? "#2A9D8F" }}>{entry.full_name.charAt(0)}</div>
                    }
                  </div>
